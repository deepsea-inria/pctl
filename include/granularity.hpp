...
public:
  ...
  std::map<int, double> report_points;
  bool recorded;
#endif
...
//public:
  ...
  estimator(std::string name, bool recorded = true)
  : name(name) {
    init();
#ifdef REPORTS
...
#ifdef REPORTS
  if (estimator::recorded) {
    if (estimator::report_points.size() == 0) {
      return;
    }
    std::ofstream output;

    output.open("reports/" + estimator::name + ".txt");
    output << "complexity time" << "\n";
    for (auto p : estimator::report_points) {
      if (p.first != 0)
        output << p.first << " " << p.second / p.first << "\n";
    }
    output.close();
  }
#endif
...
void cstmt_sequential_with_reporting(complexity_type m,
  ...
  if (estimator.recorded) {
    ...
    estimator.report_points[comp] =
      estimator.report_points.count(comp) != 0 ? (7 * estimator.report_points[comp] + elapsed) / 8 :
        elapsed;
  }
#endif
...
void cstmt(control_by_prediction& contr,
           ...
           const Par_body_fct& par_body_fct,
           const Seq_body_fct& seq_body_fct) {
  complexity_type ct = complexity_measure_fct();
/*  if (rand() % 10 < 2 || ct <= 2) {
    cstmt_sequential_with_reporting(ct, seq_body_fct, contr.get_estimator());
  } else {
